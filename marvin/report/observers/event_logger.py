...
class EventLogger(object):
    ...
    def __init__(self, publisher, dest=sys.stdout):
        ...
        publisher.subscribe(
            self.collect_iteration_status, E.TEST_ITERATION_ENDED)
    ...
    def collect_iteration_status(self, event):
        self._current_test["iterations_count"][event.status] += 1
        self._current_test["iterations"].append(event)
        if event.status != Status.PASS and event.status != Status.SKIP:
    ...
    def on_phase_ended(self, event):
        ...
        self._p("%s[%s - %s]%s (%s)",
                INDENT, phase_header, status, phase_suffix,
                self._format_duration(event.duration))
    ...
    def on_test_ended(self, event):
        ...
        self._p("[%s - %s] %s (%s)",
                test_header, status, test_script.name,
                self._format_duration(event.duration))
    ...
    def on_suite_ended(self, event):
        ...
        for test in self._suite_status:
            ...
            ipass = test['iterations_count'][Status.PASS]
            ifail = test['iterations_count'][Status.FAIL]
            iskip = test['iterations_count'][Status.SKIP]
            ...
            for iteration in test["iterations"]:
                iter_status = self._colored_status(iteration.status)
                iter_name = iteration.iteration.name if iteration.iteration.name is not None else test['name']
                self._p("    [%s] - %s (%s)", iter_status, iter_name, self._format_duration(iteration.duration))
